...
unsigned int getSubHeight(const std::string &hname, unsigned r, unsigned scale)
{
	return (sfw::getTextureHeight(getTexture(hname)) / r) * scale;
}

unsigned int getSubWidth(const std::string &wname, unsigned c, unsigned scale)
{
	return (sfw::getTextureWidth(getTexture(wname)) / c) * scale;
}